 ...
 */
Notifications.prototype.read = function readNotification(uri) {
  return new Promise((resolve, reject) => {
    this.client.mlClient.documents.read(uri)
    .result((res) => {
      result = parser.parse(res[0].content, { ignoreNameSpace : true });
      resolve(result);
    });
  });
}
